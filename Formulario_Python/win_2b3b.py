...
boxText = "Archivo de Excel (DUTY Analysis): "
buttonOption1 = "Atrás"
...
class Window(QDialog):
    def __init__(self):
        ...
        # Editable box
        self.lineEdit = QLineEdit(variables.__path_duty_analysis__, self)
    ...
    def nextPage(self):
        # This goes to the next window only the selected file meets the specification.
            # "the name contains..."
        if "duty analysis" in variables.__path_duty_analysis__.lower():
            # "the sheets and columns are..."
            if self.checkColumnNames(variables.__path_duty_analysis__, variables.__col_duty_analysis__, ['ORIGINAL']):
                print("\t   PathFile DUTY Analysis: " + variables.__path_duty_analysis__)
                # Move file(from, to)
                try:
                    os.rename(variables.__path_duty_analysis__, variables.__path_duty_analysis_to_move__+"\\"+variables.__path_duty_analysis__.split("/")[-1])
                    print("\t   File moved to: " + variables.__path_duty_analysis_to_move__)
...
El Excel debe tener la siguiente hoja y columnas correspondientes (en el orden presentado):
    - Planilla "ORIGINAL": {variables.__col_duty_analysis__["ORIGINAL"]}
    """)
    # - Planilla "PO": {variables.__col_duty_analysis__["PO"]}
    # - Planilla "RD": {variables.__col_duty_analysis__["RD"]}
    # - Planilla "Consolidates": {variables.__col_duty_analysis__["Consolidates"]}
    # - Planilla "W4": {variables.__col_duty_analysis__["W4"]}
    # """)
        messageBox.exec()

    def skipExcel(self):
        if variables.__path_apportionment__ == "Seleccione el archivo que desea.":
            self. alertSkipExcel()
        else:
            print("\t   PathFile DUTY Analysis: File Skiped")
            variables.__path_duty_analysis__ = "Seleccione el archivo que desea."
            self.cams = win_2b3c.Window()
            self.cams.show()
            self.close()

    def alertSkipExcel(self):
        messageBox = QMessageBox()
        messageBox.setWindowTitle("Error al Omitir el archivo.")
        messageBox.setText("No puede omitir este paso si también ha omitido el paso anterior.")
        messageBox.exec()
    ...
    def browseFile(self):
        fname=QFileDialog.getOpenFileName(self, 'Open file', variables.__path_duty_analysis__, 'Excel files(*.xlsx *xls)')
        if fname[0] != "":
            self.lineEdit.setText(fname[0])
        variables.__path_duty_analysis__ = self.lineEdit.text()
