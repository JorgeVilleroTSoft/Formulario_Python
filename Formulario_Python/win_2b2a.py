...
class Window(QDialog):
    def __init__(self):
        ...
        self.buttonInGridLayout("Siguiente", 3, 3, layout)
        # Button Back
        self.buttonInGridLayout("Atrás", 3, 1, layout)
    ...
    def nextPage(self):
        ...
        if "lc duty" in variables.__path_lc_duty_bv__.lower():
            ...
            if self.checkColumnNames(variables.__path_lc_duty_bv__, variables.__col_lc_duty__, ['ORIGINAL']):
                ...
                # Move file(from, to)
                try:
                    os.rename(variables.__path_lc_duty_bv__, variables.__path_lc_duty_bv_to_move_close__+"\\"+variables.__path_lc_duty_bv__.split("/")[-1])
                    print("\t   File moved to: " + variables.__path_lc_duty_bv_to_move_close__)
...
Las Columnas en cada plantilla deben ser (en el orden presentado):
